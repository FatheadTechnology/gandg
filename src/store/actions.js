...
const client = algoliasearch('S7MN0CIBBE', 'ecf8c6a506e3515c1400eb7086879aa2', {protocol: 'https:'});
...
export const getPatternInfo = ({commit, state}, patternParams) => {
    console.log('getPatternInfo Start')
    state.pdpColors = []
    state.quantitySelected = 1
    api.getPatternInfo(patternParams.pattern).then((response) => { //get master product matching the product param in URL
        ...
        var colorList = []
        for (var i = 0; i < products.length; i++) { //loop through all the children products to find all unique colors available for the product
            if (!colorList.includes(products[i].ColorWay.ColorWayName)) {
                colorList.push(products[i].ColorWay.ColorWayName)
                console.log('products[i].ColorWay being put into pdpColors:', products[i].ColorWay)
        ...
        if (patternParams.colorway && patternParams.material && patternParams.size) { //if all three params are available, use them for pickers
        ...
        } else { //otherwise, direct to URL with the default selections
            router.replace({ name: "PDP Full Params", params: {colorway: state.colorSelected, material: state.materialSelected, size: state.sizeSelected}})
        }
...
export const findPdpProduct = ({state, commit}) => {
    for (var i = 0; i < state.patternInfo.Children.length; i++) {
        if (state.patternInfo.Children[i].Material == state.materialSelected && state.patternInfo.Children[i].ColorWay.ColorWayName == state.colorSelected && state.patternInfo.Children[i].SizeDisplayName == state.sizeSelected) {
